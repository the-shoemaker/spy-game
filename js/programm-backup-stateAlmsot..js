...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function loadTopicsScript() {
      return new Promise(resolve => {
        ...
        script.onload = () => {
          // Wähle globales Objekt je nach Sprache
          topicsData = (currentLanguage === 'DE' ? window.topics : window.topics_en) || {};
    ...
    changeLangBtn.addEventListener('click', async () => {
      // Sprache wechseln und alte Themen-Auswahl löschen
      currentLanguage = currentLanguage === 'DE' ? 'EN' : 'DE';
      deSpan.classList.toggle('active', currentLanguage === 'DE');
      enSpan.classList.toggle('active', currentLanguage === 'EN');
      // Remove stale selection so new language loads fresh categories
      selectedCategories = null;
      localStorage.removeItem('selectedTopics');
      // Save both topicLanguage and translation language for consistency
      saveLanguage();
      localStorage.setItem('language', currentLanguage.toLowerCase());
      // Lade neues topics-Skript und aktualisiere Liste
    ...
    function pickNewTopicSession() {
      ...
      if (!avail.length) {
        showAlert('Keine Themen mehr, Session wird zurückgesetzt').then(() => { resetSession(true); pickNewTopicSession(); });
        return;
      }
    ...
    actionButton.addEventListener('click', () => {
      if (actionState === 'reveal') { revealItem(); setActionState(currentPlayer < numPlayers ? 'next' : 'end'); }
      else if (actionState === 'next') nextTurn(); else resetGame();
    });
    ...
    function toggleSessionMode() {
      if (!isSessionMode) {
        isSessionMode = true; sessionRoundPlayed = false;
        sessionTopics = Object.fromEntries(Object.entries(window.topics).map(([k, v]) => [k, [...v]]));
        saveSessionState();
      } else {
        isSessionMode = false; sessionRoundPlayed = false; sessionTopics = {}; localStorage.removeItem('sessionState');
      }
      updateSessionIndicator(); updateSessionButtons();
    }
    function resetSession(auto = false) {
      isSessionMode = true; sessionRoundPlayed = auto;
      sessionTopics = Object.fromEntries(Object.entries(window.topics).map(([k, v]) => [k, [...v]]));
      saveSessionState(); updateSessionIndicator(); updateSessionButtons();
    }
